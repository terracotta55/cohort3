...
describe("testing callback functions", () => {
	test("testing twoWesternProvs fn works on its own", () => {
		expect(functions.twoWesternProvs(people, resultsArr => resultsArr)[7].province).toBe("AB");
		expect(functions.twoWesternProvs(people, resultsArr => resultsArr)[8].province).toBe("BC");
	});
	...
	test("testing twoWesternProvs calling twoProvsCallback", () => {
		expect(functions.twoWesternProvs(people, functions.twoProvsCallback)[12]).toBe("Latora Matthews");
		expect(functions.twoWesternProvs(people, functions.twoProvsCallback)[19]).toBe("Seun Kelly");
	});
});
...
describe("testing company email builder functions", () => {
	...
	test("testing greater than 1000 using filter callback", () => {
		expect(functions.staffGreaterThanBalance(data.staff)).toEqual([{ fname: "Emma", lname: "Jones", balance: 1330 }]);
	});
	// 29-Oct-2019
